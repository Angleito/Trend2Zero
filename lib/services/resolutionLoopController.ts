...
import { AutoFixModule, AutoFixResult } from './autoFixModule.js';
import { PreCommitHookRunner, PreCommitHookResult } from './preCommitHookRunner.js';
...
export type IterationReport = {
  ...
  preCommitResult: PreCommitHookResult;
...
export class ResolutionLoopController {
  ...
  async run(): Promise<IterationReport[]> {
    ...
    while (iteration < this.maxIterations) {
      ...
      if (totalErrorsAfter === 0) break;
      if (!progressMade || totalErrorsAfter === previousErrorCount) break;
...
if (process.argv[1] && process.argv[1].endsWith('resolutionLoopController.ts')) {
  (async () => {
    ...
    for (const report of reports) {
      ...
      if (report.preCommitResult.output) console.log(report.preCommitResult.output);
      if (report.preCommitResult.error) console.error(report.preCommitResult.error);
