...
export class Logger {
  ...
  color(msg: string, color: keyof typeof colors) {
  ...
  debug(msg: string) {
    if (this.detailed) console.log(this.color('[DEBUG] ', 'gray') + msg);
  }
  ...
  detailedReport(errors: any[], filesModified: string[]) {
    ...
    if (errors.length) {
      ...
      for (const err of errors) {
        ...
        msg += err.message;
        if (err.line) msg += ` (line ${err.line}` + (err.column ? `:${err.column}` : '') + ')';
