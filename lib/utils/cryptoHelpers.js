 ...
 */
export function isCommonStockSymbol(symbol) {
    if (!symbol)
        return false;
 ...
 */
export function isCryptoCurrency(symbol) {
    if (!symbol)
        return false;
 ...
 */
export function getCryptoOriginType(symbol) {
    if (!symbol)
        return null;
    const parts = symbol.split(':');
    if (parts.length <= 1)
 ...
 */
export function getCommonCryptoSymbols() {
