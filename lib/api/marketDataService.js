...
export const getAllAssets = async (options = {}) => {
  try {
    const response = await apiClient.get('/api/market-data/assets', { params: options });
...
export const getAssetBySymbol = async (symbol) => {
  try {
    const response = await apiClient.get(`/api/market-data/price/${symbol}`);
    return response.data;
  } catch (error) {
...
export const getHistoricalData = async (symbol, options = {}) => {
  try {
    const response = await apiClient.get(`/api/market-data/historical/${symbol}`, { 
      params: options
    });
    return response.data;
  } catch (error) {
...
export const searchAssets = async (query, type = 'all', limit = 10) => {
  try {
    const response = await apiClient.get('/api/market-data/assets', {
