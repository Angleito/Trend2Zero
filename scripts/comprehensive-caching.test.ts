...
type TestResult = {
  service: string;
  symbol: string;
  firstCall: AssetPrice | null;
  secondCall: AssetPrice | null;
  errorFirst?: any;
  errorSecond?: any;
};
...
async function testServiceCache(
  serviceName: string,
  fetchFunction: (id: string) => Promise<AssetPrice | null>,
  assetIds: string[],
  results: TestResult[]
): Promise<void> {
  console.log(`\n=== Testing ${serviceName} Cache ===`);
  for (const assetId of assetIds) {
    let result1: AssetPrice | null = null;
    let result2: AssetPrice | null = null;
    let errorFirst: any = null;
    let errorSecond: any = null;
    try {
...
function printSummary(results: TestResult[]) {
  ...
  const byService: { [service: string]: TestResult[] } = {};
...
async function runComprehensiveCacheTests() {
  ...
  try {
    ...
    await testServiceCache(
      'CoinGecko',
      coinGeckoService.getAssetPrice.bind(coinGeckoService),
      TEST_SYMBOLS.coinGecko,
      results
    );
    ...
    await testServiceCache(
      'CoinMarketCap',
      coinMarketCapService.getAssetPrice.bind(coinMarketCapService),
      TEST_SYMBOLS.coinMarketCap,
      results
    );
    ...
    await testServiceCache(
      'MetalPriceAPI',
      metalPriceService.getMetalPrice.bind(metalPriceService),
      TEST_SYMBOLS.metalPrice,
      results
    );
