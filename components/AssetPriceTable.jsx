...
const AssetPriceTable = ({ category, limit = 20, showCategory = false, assets: propAssets = [], }) => {
    ...
    if (error) {
        ...
        <button onClick={() => window.location.reload()} className="bg-[#FF9500] text-black px-4 py-2 rounded hover:bg-opacity-80">
          Retry
        </button>
      </div>);
    }
      ...
      <table className="min-w-full bg-gray-900 rounded-lg overflow-hidden">
        <thead className="bg-gray-800">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer" onClick={() => requestSort('symbol')}>
              Symbol{getSortIndicator('symbol')}
            </th>
            {showCategory && (<th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer" onClick={() => requestSort('type')}>
                Category{getSortIndicator('type')}
              </th>)}
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer" onClick={() => requestSort('priceInUSD')}>
              Price (USD){getSortIndicator('priceInUSD')}
            </th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer" onClick={() => requestSort('priceInBTC')}>
              Price (BTC){getSortIndicator('priceInBTC')}
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-800">
          {getSortedAssets().map((asset) => (<tr key={asset.symbol} className="hover:bg-gray-800 transition-colors">
              <td className="px-6 py-4 whitespace-nowrap">
                <Link href={`/asset/${asset.symbol}`} className="text-orange-500 hover:text-orange-400 transition-colors">
              ...
              {showCategory && (<td className="px-6 py-4 whitespace-nowrap text-gray-300">
                  {asset.type || '-'}
                </td>)}
              <td className="px-6 py-4 whitespace-nowrap text-gray-300">
