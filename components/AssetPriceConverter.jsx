...
const AssetPriceConverter = ({ assetSymbol }) => {
    const [assetData, setAssetData] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [amount, setAmount] = useState(1);
    const [conversionType, setConversionType] = useState('toBTC');
    useEffect(() => {
    ...
    if (loading) {
        return (<div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
    ...
    if (error || !assetData) {
        ...
        <div className="text-center py-4">
          ...
          <button onClick={() => { if (typeof window !== 'undefined')
            window.location.reload(); }} className="px-4 py-2 bg-[#FF9500] text-white rounded hover:bg-opacity-90 transition-colors">
            Retry
          </button>
        </div>
      </div>);
    ...
    return (<div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
      ...
      <div className="mb-4">
        ...
        <input type="number" value={amount} onChange={handleAmountChange} className="w-full bg-gray-800 border border-gray-700 rounded-md px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#FF9500] focus:border-transparent" min="0" step="0.01"/>
      </div>

      <div className="flex justify-center mb-4">
        <button onClick={toggleConversionType} className="flex items-center justify-center w-10 h-10 bg-gray-800 rounded-full hover:bg-gray-700 transition-colors">
