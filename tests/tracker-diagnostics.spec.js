...
test('Diagnose Tracker Page Issues', async ({ page }) => {
    ...
    try {
        await page.waitForResponse(response => response.url().includes('/api/market-data') &&
    ...
    try {
        await page.waitForResponse(response => response.url().includes('/api/crypto/bitcoin-price') &&
            response.status() === 200, { timeout: 3000 });
