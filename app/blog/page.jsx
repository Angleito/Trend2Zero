    ...
    return (<div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <main className="flex-grow p-6">
        <div className="container mx-auto max-w-4xl">
          ...
          <StrapiContent>
            {(posts) => (<div className="grid gap-8">
                {posts.map(post => (<article key={post.slug} className="bg-gray-900 border border-gray-800 rounded-lg p-6 hover:shadow-lg transition-shadow">
                    ...
                    <div className="flex justify-between items-center">
                      {post.author && (<span className="text-sm text-gray-400">By {post.author}</span>)}
                      <Link href={`/blog/${post.slug}`} className="text-[#FF9500] hover:underline">
                        Read more →
                      </Link>
                    </div>
                  </article>))}
