...
interface ParameterProcessingLog {
  stage: string;
...
export async function GET(
  ...
  try {
    ...
    const processingLog: ParameterProcessingLog[] = [
      {
        stage: 'initial',
 ...
 */
function selectApiService(): ApiService {
  // Get all enabled services
  const enabledServices = Object.values(apiServices).filter(service => 
    service.enabled && !service.rateLimitReached
  );
 ...
 */
async function fetchFromCoinMarketCap(symbol: string, days: number): Promise<HistoricalDataPoint[]> {
  try {
    ...
    const currentResponse = await axios.get<{
      data: Record<string, { quote: { USD: { price: number; volume_24h: number } } }>
    }>(
 ...
 */
async function fetchFromCoinGecko(symbol: string, coinGeckoId: string, days: number): Promise<HistoricalDataPoint[]> {
  try {
    ...
    const historicalData: HistoricalDataPoint[] = response.data.prices.map(([timestamp, price], index) => ({
 ...
 */
function generateMockHistoricalData(symbol: string, days: number): HistoricalDataPoint[] {
 ...
 */
function extractSymbolFromParams(paramValue: SymbolParam, processingLog: ParameterProcessingLog[]): string {
  ...
  if (paramValue !== null && typeof paramValue === 'object') {
    processingLog.push({
      ...
      originalValue: paramValue,
      notes: `Object keys: ${Object.keys(paramValue as Record<string, unknown>).join(', ')}`
    });
    
    // Check for common property names that might contain the symbol
    const objValue = paramValue as Record<string, unknown>;
