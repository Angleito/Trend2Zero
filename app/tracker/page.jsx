...
export default function TrackerPage() {
    ...
    const categories = ['Stocks', 'Indices', 'Cryptocurrency', 'Commodities'];
    ...
    return (<div className="min-h-screen flex flex-col" data-testid="tracker-page">
      ...
      <main className="flex-grow p-6">
        <div className="container mx-auto max-w-6xl">
          ...
          {/* Mock Data Warning */}
          {mockDataWarning && (<div className="bg-yellow-600 text-white p-4 rounded-md mb-8 text-center" data-testid="mock-data-warning">
          ...
          <div className="mt-4 p-4 bg-gray-800 rounded-lg mb-6" data-testid="crypto-reference">
            ...
            <div className="grid grid-cols-5 gap-2">
              {TOP_CRYPTOCURRENCIES.map(symbol => (<div key={symbol} className="bg-gray-700 p-2 rounded text-center">
          ...
          {/* Error Handling */}
          {marketData.error && (<div className="bg-red-600 text-white p-4 rounded-md mb-8 text-center" data-testid="error-message">
              <p>{marketData.error}</p>
              <button onClick={marketData.refetch} className="mt-4 bg-[#FF9500] text-black px-4 py-2 rounded hover:bg-opacity-80" data-testid="retry-button">
                Retry
              </button>
            </div>)}
          ...
          {!marketData.loading && !marketData.error && (<>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 mt-6">
                <div className="md:col-span-2 overflow-x-auto">
                  <div className="flex space-x-4 pb-2">
                    {[...categories, null].map((category) => (<button key={category || 'all'} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${selectedCategory === category
                    ? 'bg-[#FF9500] text-white'
                    : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`} data-testid={`category-btn-${category ? category.toLowerCase() : 'all'}`}>
              ...
              {!marketData.popularAssets || marketData.popularAssets.length === 0 && (<div className="text-center bg-gray-800 p-8 rounded-lg mt-8" data-testid="no-assets-message">
              ...
              {marketData.popularAssets && marketData.popularAssets.length > 0 && (<AssetPriceTable assets={marketData.popularAssets} showCategory={selectedCategory === null} data-testid="asset-price-table"/>)}
            </>)}
